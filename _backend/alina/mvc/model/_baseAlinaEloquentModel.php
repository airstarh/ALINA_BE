...
class _baseAlinaEloquentModel {
	...
	public function getChildrenThrough($glueModelName, $childrenModelName, $refKeys = null, $childCols = [], $orderBy = null) {
		/** @var $children static */
		/** @var $glue static */


		$children = new $childrenModelName();
		$glue     = new $glueModelName();

		$pkNameOfParentInGlue = $this->pkName;
		$pkNameOfChildInGlue  = $children->pkName;
		if ($refKeys) {
			list($pkNameOfParentInGlue, $pkNameOfChildInGlue) = $refKeys;
		}
		...
		$q
			->select($childCols)
			->addSelect(["glue.{$pkNameOfChildInGlue} AS parent_id", "glue.{$pkNameOfParentInGlue}  AS child_id"])
			->join("{$glue->table}  glue", "children.{$children->pkName}", '=', "glue.{$pkNameOfChildInGlue}")
			->join("{$this->table}  parent", "parent.{$this->pkName}", '=', "glue.{$pkNameOfParentInGlue}");

		$children->orderByArray($orderBy);
